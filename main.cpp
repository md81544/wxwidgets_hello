...
private:
    ...
    {
        wxMessageBox(
            "This is a simple wxWidgets example for macOS.\nVersion 3.3.1",
            "About",
    ...
    void OnOpen(wxCommandEvent&)
    {
        wxFileDialog openFileDialog(
            this, _("Open file"), "", "", "All files (*.*)|*.*", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
        if (openFileDialog.ShowModal() == wxID_CANCEL) {
            return;
        }

        wxMessageBox(
            "Selected file:\n" + openFileDialog.GetPath(), "Open", wxOK | wxICON_INFORMATION);
    }

    void OnPreferences(wxCommandEvent&)
    {
        wxMessageBox("Preferences dialog placeholder.", "Preferences", wxOK | wxICON_INFORMATION);
    }
